...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config):
        ...
        # self.bert = SentenceTransformer("mixedbread-ai/mxbai-embed-large-v1")
        self.bert = SentenceTransformer('all-MiniLM-L6-v2')
        # self.bert = SentenceTransformer("infgrad/stella_en_1.5B_v5", trust_remote_code=True)
        # self.bert = SentenceTransformer("Alibaba-NLP/gte-Qwen2-1.5B-instruct", trust_remote_code=True)
    ...
    def _load_feat(self, filepath, source):
        ...
        for field in columns:
            ...
            if ftype == FeatureType.TOKEN_SEQ:
                if str(field) == self.emb_init:
                    df[field] = df[field].apply(ast.literal_eval)
                    df[field] = df[field].apply(np.array)
                    # [np.array(ast.literal_eval(string)) for string in df[field]]
                elif str(field) == self.title:
                    pass
                else:
                    df[field] = [
    ...
    def _concat_remaped_tokens(self, remap_list):
        ...
        for feat, field, ftype in remap_list:
            if ftype == FeatureType.TOKEN:
                ...
                if str(field) in (self.emb_init, self.title):
